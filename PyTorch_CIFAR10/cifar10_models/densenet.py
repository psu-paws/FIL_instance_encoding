    ...
    def forward_for_jacobian(self, x):
        ...
        for layer in self:
            if isinstance(layer, nn.BatchNorm2d):
                ...
                new_features = new_features.squeeze()
            else:
                new_features = layer(new_features)
        if self.drop_rate > 0:
            new_features = F.dropout(
        ...
        return torch.cat([x, new_features], 0)
    ...
    def forward_for_jacobian(self, x):
        ...
        for i, layer in enumerate(self.features):
            if isinstance(layer, nn.BatchNorm2d):
                ...
                x = layer(x)
                x = x.squeeze()
            elif isinstance(layer, _DenseBlock):
                for block in layer:
                    x = block.forward_for_jacobian(x)
            elif isinstance(layer, _Transition):
                for op in layer:
