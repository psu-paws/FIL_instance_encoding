...
sys.path.append('..')
from util import calc_tr_transformer
...
class Trainer:
    ...
    def __init__(
        ...
        self.jacloss_alpha = jacloss_alpha
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.train_lb == 0:
            ...
            #inputs['inputs_embeds'] = model.forward_embs(inputs['input_ids'])
            #d = inputs['inputs_embeds'][0].flatten().shape[0]
            inputs_embeds = model.forward_embs(inputs['input_ids'])
            d = inputs_embeds[0].flatten().shape[0]
            if self.jacloss_alpha > 0.0:
                #model.set_bn_training(False)
                tr, z = calc_tr_transformer(model, inputs, inputs_embeds, model.device, jvp_parallelism=jvp_parallelism, subsample=100) # Only subsample 100 for training time
                #model.set_bn_training(True)

                sigma = torch.sqrt(self.train_lb * tr.detach().clone() / d)
                #inputs['split_activation'] = z
                outputs = model.forward_second(inputs, z, sigma=sigma)
            else:
                with torch.no_grad():
                    #model.set_bn_training(False)
                    tr, _ = calc_tr_transformer(model, inputs, inputs_embeds, model.device, jvp_parallelism=jvp_parallelism, subsample=100) # Only subsample 100 for training time
                    #model.set_bn_training(True)
                    sigma = torch.sqrt(self.train_lb * tr / d)

                z = model.forward_first(inputs_embeds, inputs['attention_mask'], for_jacobian=False)
                #inputs['split_activation'] = z
                outputs = model.forward_second(inputs, z, sigma=sigma)
